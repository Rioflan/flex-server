...
var pooledPlace_1 = __importDefault(require("../models/pooledPlace"));
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    ...
                    for (index = 0; index < length; index++) { // for each place
                        place = places[index];
                        if (place.using === true) {
 ...
 */
function addPooledPlace(id_place) {
    var pooledPlace = new pooledPlace_1.default();
    pooledPlace.id = id_place;
    return pooledPlace.save()
        .then(function (pooledPlace, err) {
        if (err)
            console.log(err);
        else
            console.log("Place " + pooledPlace.id + " added to pool");
    });
}
exports.addPooledPlace = addPooledPlace;
/**
 * This function is used to remove a place from the database's pool.
 * @param id_place the id of the place to be removed
 */
function removePooledPlace(id_place) {
    pooledPlace_1.default.deleteOne({ id: id_place }, function (err) {
        if (err)
            console.log(err);
        else
            console.log(id_place + " removed from pool");
    });
}
exports.removePooledPlace = removePooledPlace;
/**
 ...
 */
exports.getPooledPlaces = function () { return pooledPlace_1.default.find({}).then(function (pooledPlaces) { return pooledPlaces.map(function (pooledPlace) { return pooledPlace.id; }); }); };
