...
var createUser = function (req, res, hashedPassword) {
...
var isValidPassword = function (req, user) { return bcryptjs_1.default.compareSync(req.body.password, user.api_key); };
var Auth = function (router) {
    ...
    router.post('/login', function (req, res) {
        apikey_1.default.findOne({ email: req.body.email }, function (err, user) {
            ...
            if (!user)
                return res.status(404).send('No user found.');
            var passwordIsValid = isValidPassword(req, user);
            if (!passwordIsValid)
