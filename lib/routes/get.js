...
var Get = function (router) {
    ...
    router
        ...
        user_1.default.find(query, function (err, user) {
            if (err)
                res.status(400).send(err);
            var friends = user[0].friend.map(function (e) {
                user_1.default.find({ id: e.id }, function (err, friend) {
                    return {
                        id: friend[0].id,
                        name: test_1.decrypt(friend[0].name, req.userId),
                        fname: test_1.decrypt(friend[0].fname, req.userId),
