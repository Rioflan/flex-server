...
var post = function (router) {
    ...
    function updateUser(id_user, params) {
        var _this = this;
        user_1.default.findOne({ id: id_user }, null, { sort: { _id: -1 } }, function (err, user) { return __awaiter(_this, void 0, void 0, function () {
            var image;
                ...
                switch (_a.label) {
                    case 0:
                        ...
                        if (params.id_place !== null)
                            user.id_place = params.id_place;
                        if (!(params.photo &&
                            params.photo.match(HTTPS_REGEX) === null &&
                            (params.photo !== "" || params.photo !== null))) return [3 /*break*/, 2];
                    ...
                    case 3:
                        if (params.remoteDay)
                            user.remoteDay = params.remoteDay;
                        user.save(function (err) {
                            if (err)
    ...
    function updatePlace(id_place, params) {
        place_1.default.findOne({ id: id_place }, function (err, place) {
            ...
            place.save(function (err) {
                if (err)
                    RES.status(resultCodes.serverError).send(errorMessages.placeUpdate);
                console.log("Place Updated");
            });
        });
    }
    var userExists = function (body) {
        user_1.default.findOne({ id: body.id_user }, null, { sort: { _id: -1 } }, function (err, user) {
            if (err)
                return RES.status(resultCodes.serverError).send(errorMessages.userFind);
            if (!user) {
            ...
            if (user && user.name === body.name && user.fname === body.fname)
                return RES.status(resultCodes.success).send({ user: user });
            else
                return RES.status(resultCodes.serverError).send(errorMessages.userIdMatch);
                ...
                switch (_a.label) {
                    ...
                    case 1:
                        ...
                        if (!(body.photo &&
                            ...
                            (body.photo !== "" || body.photo !== null))) return [3 /*break*/, 3];
                        return [4 /*yield*/, cloudinary_1.default.uploader
