...
module.exports = function (router) {
    ...
    var encrypt = require("./test").encrypt;
    ...
    router.route('/users/last')
        .get(VerifyToken, function (req, res) {
        User.find(getQuery(req), null, { limit: 1, sort: { _id: -1 } }, function (err, user) {
            if (err)
                res.status(400).send(err);
    ...
    router.route('/users/:user_id/last')
        ...
        User.find(query, null, { limit: 1, sort: { _id: -1 } }, function (err, user) {
            if (err)
                res.status(400).send(err);
            res.status(200).json(user);
        });
