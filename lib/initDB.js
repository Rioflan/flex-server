        ...
        switch (_a.label) {
            ...
            case 1:
                ...
                return [4 /*yield*/, apikey_1.default.findOne({ email: usermail }, function (err, user) {
                        if (err)
                            return process.stdout.write('Error on the server.\n');
                        if (!user)
                            return process.stdout.write('No user found.\n');
                        var token = jsonwebtoken_1.default.sign({ id: user._id }, process.env.API_SECRET || credentials_json_1.default.api_secret);
